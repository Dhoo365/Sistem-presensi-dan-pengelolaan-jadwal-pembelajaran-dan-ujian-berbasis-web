...
router.get("/kelas-ajar", requireAuth, async (req, res) => {
  try {
    ...
    const hariStr = capitalize(dayNameManado(tanggal));
    /* ==========================
       JADWAL PELAJARAN
    ========================== */
    const { data: pelajaran, error: err1 } =
      await supabase
        .from("jadwal")
        .select(`
          id_jadwal,
...
router.get("/jadwal", requireAuth, async (req, res) => {
  try {
    ...
    /* ==========================
       JADWAL PELAJARAN
    ========================== */
    const { data: pelajaran, error: err1 } = await supabase
      .from("jadwal")
      .select(`
        id_jadwal,
        kelas,
    ...
    /* ==========================
       JADWAL UJIAN
    ========================== */
    const { data: ujian, error: err2 } = await supabase
      .from("jadwal")
      .select(`
        id_jadwal,
        kelas,
